...
std::vector<std::string> Graph::BFS(std::string sourceNode, std::string targetNode) {
    ...
    std::vector<std::string> pathVec;//stores the final path
    if (nodeMap.find(sourceNode) == nodeMap.end()) { return pathVec; } //checks if sourceNode and targetNode exist
    ...
    //BFS initializes a queue and prepares to perform BFS 
    std::queue<std::string> Q; 
    Q.push(sourceNode);
    
    //starts a while loop that continues until the queueis empty
    //goes down the nodes
    while (!Q.empty()) {
        std::string currNode = Q.front();
        Q.pop();
...
std::vector<std::string> Graph::Dijktras(std::string sourceNode, std::string targetNode) {
    ...
    if (nodeMap.find(sourceNode) == nodeMap.end()) { return pathVec; } //if sourceNode exists
